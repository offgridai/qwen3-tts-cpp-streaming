...
bool Qwen3TTSBackend::Startup(const TTSRequest& Request, TTSResponse& OutResponse)
{
    ...
    std::cout << "[OffgridAI_TTS][startup] use_gpu=" << BoolText(Request.bUseGPU)
              ...
              << " steady_tail=" << ClampPositive(Request.SteadyTailWindowFrames, 12)
...
void Qwen3TTSBackend::SynthesisThreadMain(TTSRequest Request, uint64_t Generation)
{
    ...
    Params.steady_tail_window_frames = ClampPositive(Request.SteadyTailWindowFrames, 12);
    ...
    Params.audio_chunk_callback = [this, Request, Generation, StartTime, FirstChunkSeen, CallbackSampleCount, &AudioChunkSampleRate, &EnqueuePCM16Immediate](const float* Samples, size_t NumSamples, int32_t SampleRate) -> bool
    {
...
bool Qwen3TTSBackend::RunStartupPrewarmLocked(const TTSRequest& Request, std::string& OutError)
{
    ...
    Params.audio_chunk_callback = [](const float*, size_t, int32_t) -> bool
    {
        return true;
    };
