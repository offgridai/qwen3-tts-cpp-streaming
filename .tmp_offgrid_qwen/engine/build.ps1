...
function Import-VSEnv {
    ...
    $hasCMake = $null -ne (Get-Command cmake.exe -ErrorAction SilentlyContinue)
    $hasLibEnv = -not [string]::IsNullOrWhiteSpace($env:LIB)
    if ($hasCompiler -and $hasCMake -and $hasLibEnv) {
        return
    }
    ...
    $vsroot = (& $vswhere -latest -products * -requires Microsoft.VisualStudio.Component.VC.Tools.x86.x64 -property installationPath 2>$null | Select-Object -First 1).Trim()
    ...
    $vsProgramRoot = Join-Path ([Environment]::GetFolderPath("ProgramFiles")) "Microsoft Visual Studio"
    if (Test-Path $vsProgramRoot) {
        $cmakeExe = Get-ChildItem -Path $vsProgramRoot -Recurse -Filter cmake.exe -ErrorAction SilentlyContinue | Select-Object -First 1
        $ninjaExe = Get-ChildItem -Path $vsProgramRoot -Recurse -Filter ninja.exe -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($cmakeExe) {
            $toolRoots += $cmakeExe.Directory.FullName
        }
        if ($ninjaExe) {
            $toolRoots += $ninjaExe.Directory.FullName
        }
    }

    foreach ($toolPath in $toolRoots | Select-Object -Unique) {
        if ((Test-Path $toolPath) -and -not ($env:PATH -split ';' | Where-Object { $_ -eq $toolPath })) {
...
$exePath = Find-FirstExisting @(
    (Join-Path $resolvedBuildDir "$Configuration\tts_engine_cli.exe"),
    (Join-Path $resolvedBuildDir "tts_engine_cli.exe"),
    (Join-Path $resolvedBuildDir "bin\$Configuration\tts_engine_cli.exe"),
    (Join-Path $resolvedBuildDir "bin\tts_engine_cli.exe")
)
...
if ($exePath) {
    ...
    Write-Host "CLI executable not found (target might not include tts_engine_cli)." -ForegroundColor Yellow
}

if ($RunSmokeTest) {
    if (-not $exePath) {
        throw "RunSmokeTest requested, but tts_engine_cli.exe was not found."
    }
