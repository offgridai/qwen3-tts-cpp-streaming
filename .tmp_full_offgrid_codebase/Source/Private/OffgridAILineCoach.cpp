...
void UOffgridAILineCoach::SubmitOutputAudioChunk(FName LineID, const TArray<uint8>& PCMChunk, int32 SampleRate, int32 NumChannels, int64 ChunkStartSample, int32 ChunkSampleCount)
{
    ...
    // Audio must be the first consumer on the streaming critical path. Qwen's
    // StreamingAudioPlayer writes each decoded burst to its device queue before any
    // sidecar work. Match that here: push PCM into USoundWaveProcedural and allow
    // playback to start before running lipsync analysis or runtime updates.
    FlushPendingOutputPCM(false);
    StartOutputPlaybackIfReady(false);
    ...
    // Lipsync is strictly sidecar/non-blocking. Do it after the audio enqueue/start
    // opportunity so expensive analysis cannot delay first sound or create underruns.
...
void UOffgridAILineCoach::StartOutputPlaybackIfReady(bool bForceStart)
{
    ...
    if (!bOutputPlaybackStarted)
    {
        ...
        // into the procedural stream. Do not run lipsync updates here; this is the
        // audible start critical path. The next sidecar update will consume the
        // playback/audio-buffer mapping after playback has already started.
