...
public:
    ...
    bool open(int32_t sample_rate, int64_t stream_start_ms) {
#ifdef _WIN32
        close();
        ...
        return true;
#else
        (void) sample_rate; (void) stream_start_ms;
...
tts_result pipeline_internal::ops::synthesize_internal(Qwen3TTS & self,
    ...
    if (params.streaming_generate) {
        ...
        if (params.play_streaming) {
            ...
            if (!live_player->open(sample_rate, t_generate_start)) {
        ...
        if (params.print_progress) {
            fprintf(stderr,
                    ...
                    "  first_tail_window_frames=%d steady_tail_window_frames=%d context_frames=%d final_context_frames=%d\n"
                    ...
                    first_window, steady_window, context_frames, final_context_frames);
        }
        ...
        auto process_decode_job = [&](const DecodeJob & job) -> bool {
            ...
            const int64_t decode_ms = get_time_ms() - decode_start_ms;
        ...
        auto enqueue_tail_window = [&](const std::vector<int32_t> & all_codes, int32_t end_frame, bool is_final) -> bool {
            ...
            if (job.index == 0 && first_window_queued_ms < 0) {
                first_window_queued_ms = get_time_ms() - t_generate_start;
            }
